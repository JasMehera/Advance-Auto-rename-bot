...
async def initiate_rename_command(client: Client, message: Message):
    ...
    # Basic premium check (more detailed check before adding to queue)
    is_premium = await db.is_premium_user(user_id)
    if not is_premium:
        # Check if file size exceeds non-premium limit (if any)
        if hasattr(media, 'file_size') and media.file_size > Config.NON_PREMIUM_FILE_SIZE_LIMIT: # Define this in config.py
            await message.reply_text(
                f"Your file size ({get_size(media.file_size)}) exceeds the limit for non-premium users ({get_size(Config.NON_PREMIUM_FILE_SIZE_LIMIT)}).\n\n"
                "Please upgrade to premium to rename larger files. /premium"
            )
            return
    ...
    elif original_file_message.photo:
    ...
    # Pyrogram Message objects can't be directly pickled/passed between async tasks easily.
    # We only need essential info like chat_id and message_id for replies.
    message_dict = {
        "id": original_file_message.id,
        "chat": {"id": original_file_message.chat.id},
        "caption": original_file_message.caption if original_file_message.caption else None, # Include caption
        "document": {"file_name": original_file_message.document.file_name} if original_file_message.document else None,
        "video": {"file_name": original_file_message.video.file_name} if original_file_message.video else None,
        "audio": {"file_name": original_file_message.audio.file_name} if original_file_message.audio else None,
        # Add other necessary fields if required by process_rename_task or subsequent steps
    }
